...
try {
    ...
    function get_userReco($matrix) {
        ...
        foreach ($others as $item) {
            if ($item[count($item) - 2] !== 'Je ne sais pas' && $item[count($item) - 2] !== $theitem[count($theitem) - 1] && !in_array($item[count($item) - 2], $userRecommendations)) {
                $userRecommendations[] = $item[count($item) - 2];
            }
